...
function RNN.create(input_size, num_L, num_h, opts)

    -- check input sanity
    local opts = opts or {}
    local rnn_type = opts.rnn_type or 'simplified'
    local output_size = opts.output_size or -1
...
-- create RNN, do a forward pass and plot the forward computation graph
    ...
    dummy_prev  = torch.rand(batch_size, 100)
    y = model:forward({dummy_input, dummy_prev})
    print('Single Layer RNN with Decoder Model')
    print('batch_size: '..batch_size..', layers: 1')
    print(y)
