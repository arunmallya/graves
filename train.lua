...
if string.len(opt.init_from) > 0 then
    ...
    if not torch.isTensor(opt.rnn_size) then 
        ...
        assert(opt.rnn_size:size(1) == opt.num_layers,
            'invalid rnn_size: need one scalar or a tensor of same length as num_layers')
    end
    print('creating an ' .. opt.model .. ' with ' .. opt.num_layers .. ' layers')
    protos = {}
    if opt.model == 'lstm' then
        --protos.rnn = LSTM.create(vocab_size, opt.num_layers, opt.rnn_size, opt.dropout)
        protos.rnn = LSTM.create(vocab_size, opt.num_layers, opt.rnn_size)
    elseif opt.model == 'rnn' then
        protos.rnn = RNN.create(vocab_size, opt.num_layers, opt.rnn_size, opt.dropout)
    end
